...
static struct platform_device vsnv3_gpio_leds_device = {
	.name			= "leds-gpio",
	.id     = 0,
	.dev    = {
...
void __init vsnv3_gpio_leds(struct gpio_led *leds, int nr)
{
	int i;
	...
	//for (i = 0; i < nr; i++)
	//	vsnv3_set_gpio_output(leds[i].gpio, leds[i].active_low);

	led_data.leds = leds;
	led_data.num_leds = nr;
	platform_device_register(&vsnv3_gpio_leds_device);
}
...
{
	vsnv3_set_gpio_value(led, 0);
}

static inline void at91_led_off(unsigned int led)
{
	vsnv3_set_gpio_value(led, 1);
}

static inline void at91_led_toggle(unsigned int led)
{
	unsigned long is_off = vsnv3_get_gpio_value(led);
	if (is_off)
		at91_led_on(led);
	else
		at91_led_off(led);
...
void __init at91_init_leds(u8 cpu_led, u8 timer_led)
{
	...
	vsnv3_set_gpio_output(cpu_led, 1);
	vsnv3_set_gpio_output(timer_led, 1);
