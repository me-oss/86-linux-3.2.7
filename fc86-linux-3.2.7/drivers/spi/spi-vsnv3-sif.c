				...
				struct spi_message *msg)
{
	...
	u8 			spi_cmd ;	
	u8 			spi_ctl;
	u8 			spi_ctl_data_width;
	u32 			spi_tx_addr ;
	//u8			use_cmd_reg = 0; //use AIT SIF_WR_DATA0 , SF_WR_DATA1 register instead of DMA buffer
	int i;
	...
	if (xfer&&xfer->tx_buf) {
		...
		else if(len<=4) //except some special command(currently only WRITE_STATUS). 
					   //In most case first command is  1 byte cmd id with 2 or 3 bytes address. 
					   //And second one is TX or RX xfer, 
		{
...
//		if(spi_cmd == SIF_CMD_FAST_READ_DATA||spi_cmd ==SIF_CMD_FAST_READ_DUAL_IO)
//			spi_ctl |= SIF_FAST_READ;

//		if(spi_cmd == SIF_CMD_WRITE_STATUS ||spi_cmd==SIF_CMD_PAGE_PROGRAM||spi_cmd==SIF_CMD_PAGE_QUAD_PROGRAM)
//			spi_ctl |= SIF_W;
	...
	mod_timer(&as->timer, jiffies +  msecs_to_jiffies(3000));
...
{
//	struct vsnv3_spi *as = spi_master_get_devdata(spi->master);
