...
//if(AX_READ (&ax_local->ax_spi, P0_ISR))
//	printk(KERN_INFO"P0_ISR = %x\n",AX_READ (&ax_local->ax_spi, P0_ISR));				
//printk(KERN_INFO"1TFBFCR %02x %02x %x\n",TFBFCR[0],TFBFCR[1], tx_skb->len/132);
...
//		while (((AX_READ (&ax_local->ax_spi, P0_TSNR) & TXNR_TXB_IDLE) == 0))
	//	{
//printk(KERN_INFO"4 P0_TSNR = %x\n",AX_READ (&ax_local->ax_spi, P0_TSNR) );	
...
//printk(KERN_INFO "%s.\n",__func__);
	...
	if (!(isr & (ISR_TXERR|ISR_TXPAGES|ISR_LINK|ISR_RXPKT)))
		return IRQ_NONE;
#endif
	//unsigned long	intsrc_H2L = pGPIO->GPIO_INT_CPU_EN[0] & pGPIO->GPIO_INT_H2L_SR[0];
	//if(!(intsrc_H2L&(1<<MMPF_PIO_REG_GPIO2)))
	//	return IRQ_NONE;

	// pGPIO->GPIO_INT_H2L_SR[0] = (1<<MMPF_PIO_REG_GPIO2);
...
printk(KERN_INFO "%s.\n",__func__);
	...
	MMPF_PIO_EnableGpioMode(MMPF_PIO_REG_GPIO2,1);
	MMPF_PIO_EnableOutputMode(MMPF_PIO_REG_GPIO2,0);
//	MMPF_PIO_EnableTrigMode(AIT_GPIO_AGPIO02, MMPF_PIO_TRIGMODE_EDGE_L2H, 1);
	MMPF_PIO_EnableTrigMode(MMPF_PIO_REG_GPIO2,MMPF_PIO_TRIGMODE_EDGE_H2L, 1);
	MMPF_PIO_EnableInterrupt(MMPF_PIO_REG_GPIO2, 1, 0, 0);	
...
#ifdef CONFIG_ARCH_VSNV3AIT845X		
	ndev->irq = spi->irq;//gpio_to_irq(AIT_GPIO_ETHERNET_IRQ);//GPIO_IRQ_NUM_AGPIO02;//NR_AIC_IRQS+MMPF_PIO_REG_GPIO2;//AITVSNV3_ID_GPIO;//spi->irq;
	//strcpy(ndev->name , "AX88796C");
#else			
	ndev->irq = spi->irq;
#endif
