...
#if 0	
	ait8455_i2s_snd_device[1] = platform_device_alloc("soc-audio", 11);
	if (!ait8455_i2s_snd_device[1]) {
		printk(KERN_ERR "ASoC: Platform device allocation failed\n");
		ret = -ENOMEM;
		goto err_mclk;
	}

	platform_set_drvdata(ait8455_i2s_snd_device[1] ,
			&snd_card_ait8455evb_i2s[1]);

	ret = platform_device_add(ait8455_i2s_snd_device[1] );
	if (ret) {
		printk(KERN_ERR "ASoC: Platform device allocation failed\n");
		platform_device_put(ait8455_i2s_snd_device[1]);
		
		goto err_device_add;
	}


	ait8455_i2s_snd_device[2] = platform_device_alloc("soc-audio", 12);
	if (!ait8455_i2s_snd_device[2]) {
		printk(KERN_ERR "ASoC: Platform device allocation failed\n");
		ret = -ENOMEM;
		goto err_mclk;
	}
	platform_set_drvdata(ait8455_i2s_snd_device[2] ,
			&snd_card_ait8455evb_i2s[2]);

	ret = platform_device_add(ait8455_i2s_snd_device[2] );
	if (ret) {
		printk(KERN_ERR "ASoC: Platform device allocation failed\n");
		platform_device_put(ait8455_i2s_snd_device[2]);
		goto err_device_add;
	}
#endif
	return ret;
